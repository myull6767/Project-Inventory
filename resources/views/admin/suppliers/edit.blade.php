...
@section('title', 'Edit Supplier — LJN Inventory')

@section('content')
<div class="max-w-lg">
    <h1 class="font-mono text-lg text-secondary mb-6">Edit Supplier</h1>

    <form method="POST" action="{{ route('admin.suppliers.update', $supplier) }}" class="bg-surface border border-secondary/10 rounded-lg p-6 space-y-5">
        @csrf
        @method('PUT')

        <div>
            <label for="kode_supplier" class="font-mono text-xs text-primary/70 block mb-1.5">Kode Supplier</label>
            <input type="text" id="kode_supplier" name="kode_supplier" value="{{ old('kode_supplier', $supplier->kode_supplier) }}"
